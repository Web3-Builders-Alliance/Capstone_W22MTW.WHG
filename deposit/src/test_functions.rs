...
pub fn _query_deposit_coin(deps: Deps){
...
pub fn _query_cw20_deposits(deps: Deps){
...
#[test]
   ...
   fn zero_query_deposit(){ 
    ...
    setup_contract(deps.as_mut());
    deposit_coin(deps.as_mut());
    let res = deposit_functions::query_deposit(deps.as_ref(), SENDER.to_string()).unwrap();
   ...
   fn one_query_cw20_deposit(){
    ...
    let res = deposit_functions::query_cw20_deposits(deps.as_ref(), SENDER.to_string()).unwrap();
    assert_eq!(res.deposits[0].1.count, 1);
    assert_eq!(res.deposits[0].1.owner, "sender_address");
   }
}
